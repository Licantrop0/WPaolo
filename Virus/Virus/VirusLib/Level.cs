...
namespace VirusLib
{
    ...
    public class Level
    {
       ...
       // GraphicsDeviceManager graphics;
        ...
        public Level(IServiceProvider services, SpriteBatch sb, int level, Virus virus)
        {
            spriteBatch = sb;
            _virus = virus;
        ...
        public void Update(GameTimerEventArgs gameTime)
        {
            ...
            _mainbackground.Update(gameTime.ElapsedTime);
            _firstPlanBackground.Update(gameTime.ElapsedTime);

            // update the enemies
            _enemies.ForEach(wg => wg.Update(gameTime.ElapsedTime));

            // update the bonuses
            _bonuses.ForEach(b => b.Update(gameTime.ElapsedTime));

            // udpate boss
            if (_bossContainer.Count != 0)
                _bossContainer[0].Update(gameTime.ElapsedTime);

            // animate our friend virus
            if (_virus != null)
                _virus.Update(gameTime.ElapsedTime);
            ...
            _eventsManager.ManageCurrentEvent(gameTime.TotalTime);    // total time
        }
